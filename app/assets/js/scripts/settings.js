...
document.getElementById('migration').onclick = (e) => {
    if (fs.existsSync(path.join(oldClientPath, 'uninst.exe'))) {
        ...
        alert('가져올 클라이언트(이전 클라이언트)가 존재하지 않습니다!');
    }
}
...
async function resolveShaderpacksForUI(){
    const serv = (await DistroAPI.getDistribution()).getServerById(ConfigManager.getSelectedServer())
    CACHE_SETTINGS_INSTANCE_DIR = ConfigManager.getInstanceDirectory()
...
function populateReleaseNotes(){
    $.ajax({
        url: 'https://github.com/najoan125/NF-Launcher/releases.atom',
        success: (data) => {
